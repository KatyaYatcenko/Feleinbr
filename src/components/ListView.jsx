  ...
  return (
    <div className="flex flex-col h-full relative">
      <Header
        title="Мої персонажі"
        right={
          <button onClick={onSettings} className="p-2 rounded-full active:opacity-60" style={{ background: SURFACE }}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {characters.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center gap-3 px-6">
            <div className="w-16 h-16 rounded-full flex items-center justify-center" style={{ background: rgba(buttonsColor, 0.14) }}>
              <MessageCircle size={26} color={buttonsColor} />
            </div>
            <p className="font-semibold" style={{ fontFamily: 'Unbounded, sans-serif', color: TEXT }}>
      ...
      >
        <Plus size={26} color="#0E0E12" strokeWidth={2.5} />
      </button>
    </div>
  );
}
